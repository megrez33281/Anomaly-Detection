"""
此處已經整合了建立特徵庫與分群功能
若要使用手肘法：python find_clusters_and_sort.py elbow
若要指定群數 (例如 15)：python find_clusters_and_sort.py 15
"""
import torch
import numpy as np
import os
...
def find_optimal_clusters_and_sort(epoch, k_value='elbow'):
    """
    載入模型，提取特徵，根據指定的 k_value 進行分群並排序圖片。
    k_value: 可以是 'elbow' 或一個整數。
    """
    set_seed(Config.SEED)
    device = Config.DEVICE

    print(f"為 Epoch {epoch} 進行分群，模式: {k_value}")
    print("======================================================")
    ...
    if k_value == 'elbow':
        ...
        if optimal_k is None:
            print("警告：未能自動找到最佳 K 值，將預設為 15。 সন")
            optimal_k = 15
        else:
        ...
        try:
            optimal_k = int(k_value)
            print(f"已指定分群數量 K = {optimal_k}")
        except ValueError:
            print(f"錯誤：無效的參數 '{k_value}'。請輸入 'elbow' 或一個整數。 সন")
            return
    ...
    if os.path.exists(output_base_dir):
        print(f"警告：輸出資料夾 '{output_base_dir}' 已存在，將會被覆蓋。 সন")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        k_param = sys.argv[1]
        find_optimal_clusters_and_sort(TARGET_EPOCH, k_value=k_param)
    else:
        find_optimal_clusters_and_sort(TARGET_EPOCH) # 預設使用手肘法